...
export default function Home() {
  ...
  const editTask = async (boardIndex, taskIndex, newTitle) => {
    try {
      ...
      // Refetch dos dados da API para refletir as alterações imediatamente
      const response = await axios.get("http://localhost:3333/tasks");
      const dataValues = Object.values(response.data).filter((item) => Array.isArray(item));
      const mappedData = {
        0: [],
                    ...
                    <Droppable droppableId={bIndex.toString()}>
                      {(provided, snapshot) => (
                        <div {...provided.droppableProps} ref={provided.innerRef}>
                          <div
                            className={`bg-gray-100 rounded-md shadow-md flex flex-col relative overflow-hidden ${snapshot.isDraggingOver && "bg-green-100"}`}
                          >
          ...
          <div className="bg-white p-5 rounded-md shadow-md">
            ...
            <div className="flex justify-end mt-3">
              <button
                ...
                onClick={() => {
                  removeTask(selectedRemoveBoard, selectedRemoveItem);
                }}
              >
                Remover
              </button>
        ...
        <div className="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-75">
          <div className="bg-white p-5 rounded-md shadow-md">
            ...
            <textarea
              className="border-gray-300 rounded focus:ring-purple-400 w-full"
              rows={3}
              defaultValue={boardData[selectedEditBoard].items[selectedTaskIndex].title}
              onKeyDown={(e) => {
                if (e.keyCode === 13) {
            ...
            <div className="flex justify-end mt-3">
              <button
                className="px-4 py-2 bg-blue-500 text-white rounded-md mr-3"
                onClick={() =>
                  editTask(selectedEditBoard, selectedTaskIndex, document.querySelector(".bg-gray-800 textarea").value)
                }
              >
                Salvar
              </button>
